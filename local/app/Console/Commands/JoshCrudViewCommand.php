...
class JoshCrudViewCommand extends Command {
    ...
    public function fire()
    {
        ...
        foreach ($data as $item) {
            ...
            if($item['type']=='image') {
                $showContents .= '<tr><td>'.$item['name'].'</td><td>@if($'.$crudNameSingular.'[\''.$item['name'].'\']) <a href="{{ url($'.$crudNameSingular.'[\''.$item['name'].'\']) }}" target="_blank">Current Image</a> @else No Image @endif</td></tr>';
                $showContents .= "\n\t\t\t\t\t";
            } else {
